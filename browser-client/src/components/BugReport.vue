<template>
	<n-modal :show="show" @update:show="(val: any) => $emit('update:show', val)">
		<n-card
			style="width: 600px"
			title="问题反馈"
			:bordered="false"
		...
		>
			...
			<n-form :model="formValue" :rules="rules" :size="size" ref="formRef">
				<n-form-item label="问题类型" required path="bugType">
					<n-select
						v-model:value="formValue.bugType"
				...
				<n-form-item style="display: flex; justify-content: flex-end">
					<n-space>
						<n-checkbox
							v-model:checked="formValue.anonymous"
							:disabled="!store.state.login"
							text
							type="success"
						>
							匿名反馈
